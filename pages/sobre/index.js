import Header from "@/components/Header/Header"
import { Row, Container} from "react-bootstrap"

const index = () => {
    return(
        <main>
            <Header/>
            <Container >
                <Row>
                    <h1>Jogo da Memória dos Deputados Brasileiros</h1>
                    <p>No atual cenário político brasileiro, é cada vez mais importante incentivar a participação e o interesse dos jovens pela política. Uma maneira eficaz de alcançar esse objetivo é por meio da educação e do entretenimento. Nesse contexto, a criação de um jogo da memória com os deputados brasileiros voltado para jovens se mostra como uma ferramenta relevante para promover o conhecimento político desde cedo, despertando o interesse pela atuação parlamentar e fortalecendo a consciência cidadã.</p>
                    <p>Aproximação dos jovens com a política: Os jovens muitas vezes se sentem distantes da política, seja por falta de interesse, por desconhecimento ou até mesmo por descrença nas instituições políticas. Um jogo da memória com os deputados brasileiros pode ser um meio atraente de aproximar essa parcela da população da realidade política, tornando-a mais acessível e tangível.</p>
                    <p>Estímulo ao conhecimento político: Ao jogar o jogo da memória, os jovens seriam expostos aos rostos e nomes dos deputados, o que poderia incentivar a curiosidade e o interesse em descobrir mais sobre eles. Isso poderia levar a uma busca por informações adicionais, como suas posições políticas, projetos de lei apresentados e atuação na Câmara dos Deputados. Assim, o jogo se torna uma porta de entrada para o mundo político.</p>
                    <p>Fortalecimento da consciência cidadã: A participação cidadã é essencial para o desenvolvimento de uma sociedade democrática e justa. Ao conhecerem os deputados e seus papéis na formulação de leis e na representação dos cidadãos, os jovens podem desenvolver uma maior consciência sobre o poder do voto e a importância da participação política. Dessa forma, o jogo da memória contribui para a formação de cidadãos engajados e ativos.</p>
                    <p>Estímulo ao raciocínio e à memória: Além dos benefícios políticos, o jogo da memória em si proporciona uma série de vantagens cognitivas. A prática desse tipo de jogo estimula o raciocínio lógico, a concentração e o desenvolvimento da memória, características fundamentais para o aprendizado e o desempenho acadêmico dos jovens.</p>
                    <p>A criação de um jogo da memória com os deputados brasileiros voltado para jovens apresenta-se como uma iniciativa relevante para estimular o interesse pela política, promover o conhecimento político e fortalecer a consciência cidadã. Por meio dessa ferramenta, é possível aproximar os jovens da realidade parlamentar e incentivá-los a se envolverem ativamente na construção de um país melhor. O jogo da memória se mostra, assim, como uma forma lúdica e educativa de despertar o interesse político entre as novas gerações.</p>
                </Row>
            
            </Container>
            
        </main>
    )
}
export default index